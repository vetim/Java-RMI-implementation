...
import rmi.server.objCellMatrix;

public class windowChessBoardClient extends objChessBoard implements MouseListener, MouseMotionListener
{
	...
	private int currentPlayer = 0, startRow = 0, startColumn = 0, pieceBeingDragged = 0;
	 ...
	 */
	public windowChessBoardClient (CellMatrix_IF cell,KSE_IF kse) throws RemoteException
	{	
	...
	private String getPlayerMsg () throws RemoteException
	{
		...
		if (hasWon)
		{ 	set_winner(2);
			return "Congrats " + strPlayerName[0] + ", you are the winner!";
		}
		else if (firstTime)
		{
	...
	public void set_winner(int player) throws RemoteException {
		if (player == 2) {
			JOptionPane.showMessageDialog(null, "Urime "+ strPlayerName[1]+", ju jeni fitues!");
			kse.set_winner(1);
		}
		if (player == 1) {
			JOptionPane.showMessageDialog(null, strPlayerName[1]+" nuk fituat lojen!");
		}
	}
	protected void drawExtra (Graphics g) throws RemoteException
	{
		...
		if (isDragging)
		{
			g.drawImage(imgPlayer[1][pieceBeingDragged], (currentX - 25), (currentY - 25), this);
		}		
	...
	public void newGame(int player) throws RemoteException {
		if (player == 1) {
			if (firstTime == true) {
				...
				JOptionPane.showMessageDialog(null,"filloj loja e re,ju jeni fitues");
			}
			repaint();
		} else {
			if (firstTime == true) {
